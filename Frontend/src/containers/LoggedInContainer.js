...
const [isPlaying, setIsPlaying] = useState(false);
    ...
    const handleRepeatClick = () => {
        ...
        if (soundPlayed) {
            ...
            // Update the state to reflect the restart
            setIsPaused(false);
    ...
    const handlePremiumClick = () => {
        // Navigate to the "/premium" route
        navigate('/premium');
      };
      const handleSupportClick = () => {
        // Navigate to the "/support" route
        navigate('/support');
      };
    ...
    useEffect(() => {
        if (isPlaying && soundPlayed) {
            const updatePlaybackTime = () => {
                const currentTime = soundPlayed.seek();
                setCurrentPlaybackTime(currentTime);
                setProgress((currentTime / soundPlayed.duration()) * 100 || 0);
                requestAnimationFrame(updatePlaybackTime);
            };
    
            soundPlayed.on('play', updatePlaybackTime);
            soundPlayed.on('seek', updatePlaybackTime);
    
            // Set initial playback time and progress
            setCurrentPlaybackTime(soundPlayed.seek());
            setProgress((soundPlayed.seek() / soundPlayed.duration()) * 100 || 0);
    
            return () => {
                soundPlayed.off('play', updatePlaybackTime);
                soundPlayed.off('seek', updatePlaybackTime);
            };
        } else {
            // Reset progress to 0 when not playing
            setCurrentPlaybackTime(0);
            setProgress(0);
        }
...
/>
  ...
  value={progress}
  onChange={(e) => {
    const seekValue = (e.target.value / 100) * soundPlayed.duration();
    setProgress(e.target.value);
    soundPlayed.seek(seekValue);
  }}
